...
class Teris {
    ...
    declineRegularly() {
            ...
            this.timer = setTimeout(function loop() {
                if (this.canMove.bind(this)().canMoveTop) {
    ...
    judgeHigh(i, curLeft, curTop) {
            ...
            let lefts = [],
                ...
                rightest = null
            if (!stopItem.length) {
                highest = this.layerHeight - this.itemSize;
                leftest = 0;
                ...
                highest = tops.length ? Math.min(...tops) - this.itemSize : this.layerHeight - this.itemSize;
    ...
    canMove(change, move = {
            ...
            let lefts = [],
                ...
                for (let i of moveItem) {
                    ...
                    if (parseInt(i.style.top) >= bound.highest) {
                        move.canMoveTop = false;
                        leftest.push(false);
                    } else {
                        move.canMoveTop = true;
                    }
                }
                if (leftest.includes(false)) {
                    move.canMoveTop = false;
                }
                if (Math.min(...lefts) > bound.leftest) {
                    move.canMoveLeft = true;
                }
                if (Math.max(...lefts) < bound.rightest) {
                    move.canMoverRight = true;
                }
            return move;
        }
        /**改变形状**/
            ...
            v.onmousedown = (e) => {
                ...
                switch (target.id) {
                    ...
                    case 'startBtn': //重新开始
                        ...
                        break;
                    case 'alterBtn': //改变形状
                        if (this.canMove(true).canMoveTop) {
